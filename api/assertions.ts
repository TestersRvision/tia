...
const tiaAssertion = {
  initResultsAccumulator(name: string) {
    checkAccName(name);
    resultAccumulators[name] = true;
  },

  getResultFromAccumulator(name: string) {
    checkAccName(name);
    return resultAccumulators[name];
  },

  deleteResultAccumulator(name: string) {
    checkAccName(name);
    delete resultAccumulators[name];
  },

  mergeResultToAccumulator(res, name: string) {
    checkAccName(name);
   ...
   */
  true: function checkIfTrue(condition, msg: string, mode) {
    if (condition) {
      gT.l.pass(msg, mode);
      return true;
    }
   ...
   */
  false: function checkIfFalse(condition, msg: string, mode) {
   ...
   */
  value(actVal, expVal, msg: string, mode) {
   ...
   * @returns {Boolean} comparision result.
   */
  valueStr(actVal, expVal, msg: string, mode) {
   ...
   * @returns {Boolean} comparision result.
   */
  valueNumber(actVal, expVal, msg: string, mode) {
   ...
   * @returns {Boolean} comparision result.
   */
  valueBool(actVal, expVal, msg: string, mode) {
   ...
   * @returns {boolean}
   */
  valueDeep(actVal, expVal, msg: string, mode) {
   ...
   * @return {Promise}
   */
  exceptionAsync(yieldable, expExc, mode) {
    return gT.u.execGen(yieldable).then(
      () => {
        console.log('GOOD');
        let msg;
        if (typeof expExc === 'undefined') {
  ...
  equal(val1, val2, msg1: string, msg2: string, doNotShowValues, mode) {
    if (val1 === val2) {
      if (doNotShowValues) {
  ...
  equalBool(val1, val2, msg1: string, msg2: string, doNotShowValues, mode) {
    val1 = Boolean(val1);
    val2 = Boolean(val2);
    if (val1 === val2) {
      if (doNotShowValues) {
  ...
  notEqualBool(val1, val2, msg1: string, msg2: string, doNotShowValues, mode) {
    val1 = Boolean(val1);
    val2 = Boolean(val2);
    if (val1 !== val2) {
      if (doNotShowValues) {
